...
const checkRequestsBody = (request, response, next) => {
  const {id, category, todo, priority, status, dueDate} = request.body
  const {todoId} = request.params

  if (category !== undefined) {
    categoryArray = ['HOME', 'WORK', 'LEARNING']
  ...
  if (status !== undefined) {
    statusArray = ['TO DO', 'INPROGRESS', 'DONE']
    statusIsInArray = priorityArray.includes(status)
...
app.get('/todos/', checkRequestsQueries, async (request, response) => {
  const {status = '', category = '', search_q = '', priority = ''} = request
...
app.post('/todos/', checkRequestsBody, async (request, response) => {
  const {id, todo, category, priority, status, due_date} = request
  const addTodoQuery = `
        INSERT INTO 
...
app.put('/todos/: todoId/', checkRequestsBody, async (request, response) => {
  const {todoId} = request
  const {priority, todo, category, status, dueDate} = request

  let updateTodoQuery = null
  console.log(priority, todo, category, status, dueDate)
  switch (true) {
